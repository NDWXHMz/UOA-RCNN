...
def LDIoU(boxes1: Boxes, boxes2: Boxes, denominator=0):
    area1 = boxes1.area()  # [N]
    area2 = boxes2.area()  # [M]
    inter = pairwise_intersection(boxes1, boxes2)
    ...
    center1 = boxes1.get_centers()
    center2 = boxes2.get_centers()
    
    inner_distance = ((center1[:, 0].unsqueeze(1) - center2[:, 0].unsqueeze(0)) ** 2 + (center1[:, 1].unsqueeze(1) - center2[:, 1].unsqueeze(0)) ** 2)
    
    
    inter_x1 = torch.min(boxes1.tensor[:, 0].unsqueeze(1), boxes2.tensor[:, 0].unsqueeze(0))
    inter_y1 = torch.min(boxes1.tensor[:, 1].unsqueeze(1), boxes2.tensor[:, 1].unsqueeze(0))
    inter_x2 = torch.max(boxes1.tensor[:, 2].unsqueeze(1), boxes2.tensor[:, 2].unsqueeze(0))
    inter_y2 = torch.max(boxes1.tensor[:, 3].unsqueeze(1), boxes2.tensor[:, 3].unsqueeze(0))
    outer_distance = torch.sub(inter_x2, inter_x1)**2 + torch.sub(inter_y2, inter_y1)**2   
    
    diou = iou - inner_distance / (outer_distance + 1e-7)
    return diou


def pairwise_diou(boxes1, boxes2):
    iou = pairwise_iou(boxes1, boxes2) # [len(boxes1), len(boxes2)]
    center1 = boxes1.get_centers()
    center2 = boxes2.get_centers() # [1000, 2]
...
class ROIHeadsLogisticGMMNewLimitFPEnergy(ROIHeadsLogisticGMMNew):
    ...
    def export_neg_box(self, gt_boxes, proposal_boxes, iou_with_gt, match_gtid, gt_num, e2):
        ...
        gt_ratio = LDIoU(gt_boxes, neg_proposal_boxes, 0)  # inter / gt area
        for i in range(gt_ratio.shape[0]):
            gt_min, gt_max = torch.min(gt_ratio[i]), torch.max(gt_ratio[i])
            gt_ratio[i] = (gt_ratio[i] - gt_min) / (gt_max - gt_min + 1e-7)
        
        
        pro_ratio = LDIoU(gt_boxes, neg_proposal_boxes, 1)  # inter / proposal area
        for i in range(pro_ratio.shape[0]):
            pro_min, pro_max = torch.min(pro_ratio[i]), torch.max(pro_ratio[i])
            pro_ratio[i] = (pro_ratio[i] - pro_min) / (pro_max - pro_min + 1e-7)
    ...
    def sample_localboxes_and_completeboxes_t2(self, proposals, targets):
        ...
        for i, img in enumerate(proposals):
            ...
            iou_matrix = pairwise_diou(gt_boxes, proposal_boxes)
            
            
            iou_with_gt, match_gtid = iou_matrix.max(0)
            iou_max, iou_min = torch.max(iou_with_gt), torch.min(iou_with_gt)
            iou_with_gt = (iou_with_gt - iou_min) / (iou_max - iou_min + 1e-7)
